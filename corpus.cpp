...
double Corpus::calculate_bm25_for_doc(std::string key, std::string doc_key) {
  const Node *n = this->index.find_exact_match(key);
  double k = 1.25;
  double b = 0.75;
...
vector<QueryResult> Corpus::bm25(vector<std::string> query) {
  ...
  for (std::string &word : query) {
    const Node *n = this->index.find_exact_match(word);
    if (n == nullptr) {
      continue;
    }
    TermData td = n->get_leaf();
...
const Node *Corpus::find_node(std::string query) {
